"""docsynth command line — generate and export runs.
...
nothing else changes.
    ...
    docsynth generate --run-id r1 --total 1000
    docsynth export   --run-id r1 --sink duckdb:///out/golden.db
    docsynth status   --run-id r1
...
import docsynth.packs  # noqa: F401  — registers the built-in packs
from docsynth.core import RunState, WorkUnitState, available_packs, get_pack
from docsynth.core.logging import get_logger
...
from docsynth.core.selection import Selection
from docsynth.core.sinks import open_sink
from docsynth.core.state import open_state
from docsynth.core.storage import open_store
from docsynth.core.usage import DEFAULT_USAGE_URI, open_usage_sink
...
def _init(ctx: typer.Context) -> None:
    ...
    a pipe/Cloud Run; DOCSYNTH_LOG_LEVEL / DOCSYNTH_LOG_FORMAT override.

    A bare ``docsynth`` in a terminal launches the studio wizard; piped (no TTY) it
    prints help, the conventional no-args behaviour for a script."""
    from docsynth.core.logging import configure
    configure()
    if ctx.invoked_subcommand is not None:
        return
    from docsynth.studio.prompts import is_interactive
    if is_interactive():
        _run_studio()
    else:
...
_STORAGE = typer.Option(
    "./out/blobs", envvar="DOCSYNTH_STORAGE", help="Blob store URI for documents + shards"
)
_STATE = typer.Option("./out/runs.db", envvar="DOCSYNTH_STATE", help="Run-state store URI")
...
_CATALOGUE = typer.Option("", "--catalogue", envvar="DOCSYNTH_CATALOGUE",
...
def plan(
    ...
    (`docsynth status`) before committing compute to it.
    """
    if pack not in available_packs():
        raise typer.BadParameter(
            f"unknown pack {pack!r}; available: {', '.join(available_packs())}"
...
def _llm_build(providers: str, *, companies: int, products_per_company: int,
    ...
    lets a Cloud Run job carry the mix in an env var (`DOCSYNTH_PROVIDERS`) — the
    ...
    from docsynth.core.providers.factory import build_mix
    from docsynth.packs.invoice.llm_build import build_llm_catalogue_sync

    # `providers` is a file path or the mix inline (from DOCSYNTH_PROVIDERS on
...
def catalogue(
    ...
    providers: str = typer.Option(
        "", "--providers", envvar="DOCSYNTH_PROVIDERS",
        help="Provider-mix file OR inline YAML (env DOCSYNTH_PROVIDERS) → build "
    ...
    thousands of distinct descriptions while `docsynth generate` stays local-first
    and deterministic.
    ...
    from docsynth.packs.invoice.artifact import write_catalogue
    from docsynth.packs.invoice.procedural import generate_catalogue
    from docsynth.packs.invoice.validation import validate
    ...
    typer.echo(f"generate with:  docsynth generate --catalogue {out} …")


@app.command()
def export(
    run_id: str = typer.Option(..., help="Run to export"),
    sink: str = typer.Option("./out/golden", envvar="DOCSYNTH_SINK",
...
def _pdfs_progress(project, step, args):
    ...
    from docsynth.studio import Step
...
def _run_studio(*, provider: str = "", project: str = "", step: str = "", pack: str = "",
    ...
    interactive `docsynth` both call this. Walks target → project → pack → step →
    args, prompting only for what a flag left unset, with back/exit navigation."""
    from functools import partial

    from docsynth.studio import Registry, Step, StudioError, available_targets, get_target, wizard
    from docsynth.studio.app import run_step
    from docsynth.studio.progress import drain_stdin, run_with_spinner
    from docsynth.studio.prompts import BACK, EXIT, get_prompter, is_interactive
    ...
    while True:                               # ── target + project screens ──
        ...
        while True:                           # ── step menu (returns here after each run) ──
            ...
            typer.echo("  plan\n    " + (preview.command or "docsynth " + " ".join(preview.argv)))
            if dry_run:
            ...
            if (result.ok and result.run_id and step_enum is Step.PDFS
                ...
                reattach = (f"docsynth studio status -p {provider_name} "
...
def studio(
    ...
    returns handles + links immediately — follow it with `docsynth studio status`.
...
def studio_status(
    ...
    from docsynth.studio import Registry, StudioError
    from docsynth.studio.app import status_of
    try:
...
def studio_projects() -> None:
    """List the saved projects (from ~/.docsynth/projects.yaml)."""
    from docsynth.studio import Registry
    reg = Registry()
    projects = reg.projects()
    if not projects:
        typer.echo("  no saved projects — run `docsynth studio` to create or onboard one")
        return
...
def studio_provision(
    ...
    from docsynth.studio import Registry, StudioError, get_target
    from docsynth.studio.types import ProjectSpec
    reg = Registry()
    try:
...
def studio_teardown(
    ...
    from docsynth.studio import Registry, StudioError, get_target
    from docsynth.studio.prompts import get_prompter, is_interactive
    reg = Registry()
    try:
        proj = _resolve_saved_project(reg, project, provider)
        target = get_target(proj.target)
...
def _resolve_saved_project(registry, project: str, provider: str):
    ...
    from docsynth.studio import StudioError
    if ":" in project:
        ref = project
        ...
        if not matches:
            raise StudioError(f"no saved project with id {project!r}; "
                              "run `docsynth studio` to create or onboard it")
    ...
    if proj is None:
        raise StudioError(
            f"no saved project {ref!r}; run `docsynth studio` to create or onboard it"
        )
    return proj
