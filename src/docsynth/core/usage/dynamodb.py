...
single-table shape as :mod:`docsynth.core.state.dynamodb` — partition key ``pk``,
...
minutes. Prefer :mod:`docsynth.core.usage.shard` for the fact table and reach for
        ...
        *,
        ...
        if resource is None:
            try:
                ...
                raise ImportError(
                    "dynamodb:// usage needs the AWS extra — pip install 'docsynth[aws]'"
                ) from exc
            kwargs: dict[str, Any] = {}
            if region:
