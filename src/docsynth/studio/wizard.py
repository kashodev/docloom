...
from docsynth.core import available_packs
from docsynth.studio.app import resolve_project
from docsynth.studio.prompts import BACK, EXIT, Choice, Prompter
from docsynth.studio.registry import Registry
from docsynth.studio.targets import available_targets
...
def choose_project(
    ...
    if cloud:      # let an existing project point at its real region/bucket
        ...
        if region == BACK:
            return BACK
        bucket = prompter.text(
            "Bucket", default=bucket or f"{name}-docsynth", allow_back=allow_back
        )
        if bucket == BACK:
            return BACK
