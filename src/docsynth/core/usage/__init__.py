"""LLM usage telemetry: one protocol, backends chosen by URI scheme.
    ...
    firestore://project/database  Firestore collection  (pip install 'docsynth[gcp]')
    dynamodb://table              DynamoDB table        (pip install 'docsynth[aws]')
...
from docsynth.core.usage.base import (
    TABLE,
    LlmUsage,
    MemoryUsageSink,
    NullUsageSink,
...
from docsynth.core.usage.shard import ShardUsageSink
    ...
    *,
    ...
    if scheme == "firestore":
        try:
            ...
            raise ImportError(
                "firestore:// usage needs the GCP extra — pip install 'docsynth[gcp]'"
            ) from exc
