...
One YAML file (``~/.docsynth/projects.yaml``, or ``$DOCSYNTH_HOME``). It is a
...
from docsynth.studio.types import Project


def home() -> Path:
    """Where the registry lives — ``$DOCSYNTH_HOME`` or ``~/.docsynth``."""
    return Path(os.environ.get("DOCSYNTH_HOME") or (Path.home() / ".docsynth"))
