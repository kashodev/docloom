...
# ── Product slots, per business type ────────────────────────────────────────
...
_SLOTS: dict[BusinessType, dict[str, tuple[str, ...]]] = {
    BusinessType.RETAIL: {
...
BUSINESS_TYPES: tuple[BusinessType, ...] = tuple(_SLOTS)


def combination_space(business_type: BusinessType) -> int:
    """How many distinct descriptions the slots can express for a type."""
    slots = _SLOTS[business_type]
    total = 1
...
def generate_products(
    rng: Random, business_type: BusinessType, count: int
) -> list[ProductTemplate]:
    """``count`` distinct products for one company.
    ...
    """
    slots = _SLOTS[business_type]
    space = combination_space(business_type)
    if count > space:
        raise ValueError(
            f"{business_type} can express {space} distinct products, {count} asked for — "
