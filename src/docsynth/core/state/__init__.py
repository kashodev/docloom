"""Run state: one protocol, backends chosen by URI scheme.
    ...
    firestore://project/database  Firestore  (pip install 'docsynth[gcp]')
    dynamodb://table              DynamoDB   (pip install 'docsynth[aws]')
...
from docsynth.core.state.base import Run, StateStore, WorkUnit
...
def open_state(uri: str) -> StateStore:
    ...
    if scheme == "firestore":
        try:
            ...
            raise ImportError(
                "firestore:// state needs the GCP extra — pip install 'docsynth[gcp]'"
            ) from exc
