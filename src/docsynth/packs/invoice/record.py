...
This module is the contract between ``docsynth.generate`` (writer) and
``docsynth.export`` (reader). It is imported by both so the two cannot drift.
...
from docsynth.core.enums import DocumentCondition, Jurisdiction
from docsynth.core.locale.enums import Currency, Language, Locale
from docsynth.core.money import ZERO, money, sum_money
from docsynth.core.record import TableRows
...
class GoldenInvoice(_Base):
    ...
    #: The issuer's finer specialty (e.g. "women's activewear"), a sub-slice of
    #: ``product_category``, so a corpus is queryable by niche. Empty for the seed
    #: catalogue and procedural-only corpora that record no niche.
    llm_niche: str = ""
    ...
    def record_id(self) -> str:
        """Kernel-facing identifier (:class:`~docsynth.core.record.GoldenRecord`).
