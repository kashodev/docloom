...
boto3 is lazy-imported (an ``docsynth[aws]`` extra) and the table resource is
...
from docsynth.core.state.base import (
...
class DynamoDbStateStore:
    """A :class:`~docsynth.core.state.base.StateStore` backed by one DynamoDB table."""
        ...
        *,
        ...
        if resource is None:
            try:
                ...
                raise ImportError(
                    "dynamodb:// state needs the AWS extra — pip install 'docsynth[aws]'"
                ) from exc
            kwargs: dict[str, Any] = {}
            if region:
