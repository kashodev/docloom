...
class Step(StrEnum):
    ...
    CATALOG = "catalog"     # build the content catalogue (docsynth catalogue)
    PDFS = "pdfs"           # render documents + golden set (docsynth generate)
    EXPORT = "export"       # golden shards → a queryable sink (docsynth export)
...
class Result:
    ...
    ok: bool
    summary: str
    argv: tuple[str, ...] = ()      # the underlying CLI tokens (local: the `docsynth` verb + flags)
