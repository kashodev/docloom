...
* ``DOCSYNTH_LOG_LEVEL``  — ``debug`` | ``info`` (default) | ``warning`` | ``error``
* ``DOCSYNTH_LOG_FORMAT`` — ``console`` | ``json`` | ``auto`` (default: console at
...
#: Level names → stdlib levels, so ``DOCSYNTH_LOG_LEVEL`` is forgiving of case.
_LEVELS = {
...
def _resolve_level(explicit: str | None) -> int:
    name = (explicit or os.environ.get("DOCSYNTH_LOG_LEVEL") or "info").lower()
    return _LEVELS.get(name, logging.INFO)


def _use_json(explicit: str | None) -> bool:
    fmt = (explicit or os.environ.get("DOCSYNTH_LOG_FORMAT") or "auto").lower()
    if fmt == "json":
        return True
    if fmt == "console":
        return False
...
def configure(*, level: str | None = None, fmt: str | None = None,
    ...
    structlog.configure(
        ...
        # Not cached: docsynth configures once at startup, so the per-call cost is
