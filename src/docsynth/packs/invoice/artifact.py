...
sell". :class:`~docsynth.packs.invoice.catalog.SeedCatalogue` answers it from
...
:func:`~docsynth.packs.invoice.catalog.derive_identity`. They are the
...
class ArtifactCatalogue:
    """A :class:`~docsynth.packs.invoice.catalog.Catalogue` backed by an artifact.
...
def load_catalogue(uri: str, *, verify: bool = True) -> ArtifactCatalogue:
    ...
    if manifest.schema_version > SCHEMA_VERSION:
        raise ValueError(
            ...
            f"but this docsynth understands up to {SCHEMA_VERSION} — upgrade docsynth"
        )
