...
The same lifecycle applies whether you drive it with the [`docloom` CLI](../core/overview.md)
locally, [`deploy.sh`](deploy-tool.md) on GCP, or the [studio](../studio/overview.md).

## Run

- **Local:** `docloom generate --run-id r1 --total 1000 [--catalogue <uri>] [--format pdf|html]`.
...
- `docloom status --run-id r1 --state <uri>` — a snapshot; `--wait` streams until
  the run is terminal.
- `docloom studio status --project <ref> --run r1 [--wait]` — reattaches to a
...
- `docloom generate … --resume` (local) or `deploy.sh resume` (cloud).
...
`docloom pause` / `docloom cancel` set the run state so workers stop claiming new
...
- `docloom export --run-id r1 --sink <uri> [--storage-prefix …]`
